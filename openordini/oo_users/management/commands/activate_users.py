        ...
        for u in User.objects.all().order_by("last_name").exclude(is_staff=True)[0:3]:
		    ...
		    email_invio.update_headers()
                ...
                try:                    
                    ...
                    mail_inviata = EmailMessage.objects.get(to_email__contains = "a.calamospecchia@gmail.com")
                    reg_test = recordo_login_by_email.objects.create(password_email=raw_password, username_email = u.username, utente_email = psicologo, ref_email = )
                    print "------------------------------------"
                    print reg_test
                    ###email_go.send()
